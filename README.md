...
As an example, we've included a small `src/lambda/hello.ts` function, which will be deployed to `/.netlify/functions/hello`. We've also included an async lambda example using async/await syntax in `async-dadjoke.ts`.
...
2. convert `src/lambda/hello.ts` to `src/lambda/hello.ts`
